...
SELECT CAT_NAME FROM Category
    ...
    INNER JOIN
		(SELECT * FROM `Order` WHERE ORD_AMOUNT=(SELECT MIN(ORD_AMOUNT) FROM `Order`)) AS Min
        ON Product.PRO_ID = Min.PROD_ID) AS Category_Id
	ON Category.CAT_ID = Category_Id.CAT_ID;
...
/* Procedure to get the Rating Verdict for ALL Suppliers */
DROP PROCEDURE If EXISTS DisplayRatingForAllSupplier;

DELIMITER //
CREATE PROCEDURE DisplayRatingForAllSupplier()
BEGIN
...
CALL DisplayRatingForAllSupplier();



/* Procedure to get the Rating Verdict for SPECIFIC Suppliers */
DROP PROCEDURE If EXISTS DisplayRatingForSpecificSupplier;
DELIMITER //
CREATE PROCEDURE DisplayRatingForSpecificSupplier(id INT)
BEGIN
	SELECT S.SUPP_ID, R.RAT_RATSTARS,
		CASE
			WHEN R.RAT_RATSTARS > 4 THEN "Genuine Supplier"
...
CALL DisplayRatingForSpecificSupplier(4);
